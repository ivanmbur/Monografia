...
Now, given that quantum mechanics seems to correctly predict the behavior of light polarization, we may go back to out previous example and test Bell's inequalities.
\begin{example}
Suppose $P_A(\theta)=P(\theta)\otimes id_{\mathbb{C}^2}$ and $P_B(\theta)=id_{\mathbb{C}^2}\otimes P(\theta)$. This means that $P_A(\theta)$ measures on the first photon and $P_B(\theta)$ on the second. More precisely we may interpret the expected value of $P_A(\theta)$ as yielding a degree of plausibility to the proposition $p(\theta)=$"\textit{the first photon has linear polarization at an angle $\theta$}" and $P_B(\theta)$ playing the analogue role for $q(\theta)=$"\textit{the second photon has linear polarization at an angle $\theta$}". Therefore we find that the degree of plausibility for the proposition $p\wedge q$ is
\begin{multline}
...
\end{multline}  				
Since the degree of plausibility of $p(\alpha)'q(\beta)'$ is given by adding $\pi/2$ to both angles we find that $P(f(p(\alpha),q(\beta)))=sin^2(\alpha-\beta)$. Therefore we obtain through Bell's inequalities
\begin{multline}
